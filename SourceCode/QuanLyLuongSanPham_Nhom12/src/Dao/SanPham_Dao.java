...
public class SanPham_Dao {
	//sửa sản phẩm
		...
		//get ds sản phẩm 
		public ArrayList<SanPham>getALLSanPham(String maSP) {
		    ArrayList<SanPham> list = new ArrayList<>();
		    ConnectDB.getInstance();
		    PreparedStatement st = null;
		    try {
		        Connection con = ConnectDB.getConnection();
		        st = con.prepareStatement("SELECT * FROM SanPham WHERE maSanPham = ?");
		        st.setString(1, maSP);
		...
		public ArrayList<SanPham> timSanPham(String maSP, HopDong maHopDong, String tenSP,  String donViTinh ,int soLuong,  String yeuCau,  double donGia){
		    ...
		    try {
		        ...
		        st.setString(5, "%" + donViTinh + "%");
		        st.setString(6, "%" + soLuong + "%");
		        st.setString(7, "%" + yeuCau + "%");
		        st.setString(8, "%" + donGia + "%");
		        list.add(sp);
...
//tìm sanpham theo mã
		public SanPham timSanPhamTheoMaSP(String maSP) {
		    ...
		    try {
		        Connection con = ConnectDB.getConnection();
		        String query = "SELECT * FROM NhanVien WHERE maSP = ?";
		        ...
		        if (rs.next()) {
		            sp = new SanPham(rs.getString("maSP"), rs.getString("tenSP"), rs.getString("donViTinh"),
		                    rs.getString("soLuong"), rs.getInt("soLuong"), rs.getDouble("donGia"), rs.getString("yeuCau"));
		        }
		    } catch (SQLException e) {
		        e.printStackTrace();
		    } finally {
